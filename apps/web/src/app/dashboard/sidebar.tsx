...
export function Sidebar({ user }: { user: User }) {
  ...
  const showAdmin = isAdminRole(role) || isSuperAdminEmail(email);
  const navItems = showAdmin ? [...baseNavItems, adminNavItem] : baseNavItems;

  const roleLabel = isAdminRole(role) || isSuperAdminEmail(email)
    ? "Administrador"
    : "Profesor";
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-2xl">
            <div className="mb-4 flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-ipade-text">Cerrar Sesion</h3>
                <p className="text-sm text-ipade-text-muted">Esta accion cerrara tu sesion actual.</p>
              </div>
            </div>
